    ...
    >
      ...
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-7rem)] max-w-7xl items-center gap-12 lg:grid-cols-[1.08fr_0.92fr]">
        <div className="space-y-8">
          <div className="inline-flex items-center gap-3 rounded-md border border-emerald-950/15 bg-[#f6f7ef] px-3 py-2 font-mono text-xs uppercase text-zinc-600 shadow-[4px_4px_0_rgba(251,191,36,0.12)] dark:border-white/15 dark:bg-black/30 dark:text-zinc-300 dark:shadow-[4px_4px_0_rgba(251,191,36,0.08)]">
            <span className="h-2 w-2 bg-amber-500 dark:bg-amber-300"></span>
            Available for selected computer engineering work
          </div>

          <div className="space-y-6">
            <p className="flex items-center gap-2 font-mono text-sm uppercase text-sky-700 dark:text-sky-300">
              <CircuitBoard className="h-4 w-4 text-amber-600 dark:text-amber-300" />
          ...
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center">
            <button 
              ...
              className="group inline-flex items-center justify-center gap-2 rounded-md bg-zinc-950 px-6 py-4 text-base font-semibold text-[#f6f7ef] transition-transform duration-200 hover:-translate-y-0.5 hover:text-amber-200 dark:bg-zinc-50 dark:text-zinc-950 dark:hover:text-amber-700"
            >
              View work log
          ...
          <div className="flex gap-3 pt-2">
            {[
              ...
              <a
                key={label}
                href={href}
                className="grid h-11 w-11 place-items-center rounded-md border border-emerald-950/15 bg-[#f6f7ef]/70 text-zinc-700 transition-colors duration-200 hover:border-amber-500/50 hover:bg-zinc-950 hover:text-amber-300 dark:border-white/15 dark:bg-black/25 dark:text-zinc-300 dark:hover:border-amber-300/45 dark:hover:bg-white dark:hover:text-amber-700"
                aria-label={label}
              >
        ...
        <div className="relative pb-16 lg:pb-0">
          <div className="rounded-lg border border-emerald-950/15 bg-zinc-950 text-[#f6f7ef] shadow-[18px_18px_0_rgba(251,191,36,0.14)] dark:border-white/15 dark:bg-[#101211] dark:shadow-[18px_18px_0_rgba(251,191,36,0.08)]">
            <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
              ...
              <span className="font-mono text-xs text-amber-300">online</span>
            </div>
...
{`const developer = {
  ...
  output: "interfaces people can operate"
              ...
              <div className="grid gap-3">
                {["Designs UI states before decoration", "Builds APIs into calm product flows", "Writes so other developers can move faster"].map((item) => (
                  <div key={item} className="flex items-center gap-3 border border-white/10 bg-white/[0.03] px-3 py-3 text-sm text-zinc-200">
                    <span className="h-2 w-2 bg-amber-300"></span>
                    {item}
                  </div>
                ))}
              </div>
        ...
        <button 
          onClick={() => scrollToSection('experience')}
          className="absolute bottom-6 left-1/2 grid h-11 w-11 -translate-x-1/2 place-items-center rounded-md border border-emerald-950/15 bg-[#f6f7ef] text-zinc-700 transition-colors hover:border-amber-500/50 hover:bg-zinc-950 hover:text-amber-300 dark:border-white/15 dark:bg-black/30 dark:text-zinc-300 dark:hover:border-amber-300/45 dark:hover:bg-white dark:hover:text-amber-700"
